...
-- (**) Generate and test for symmetric balanced trees
-- Prelude > symBalTrees 5
...
balTree n					| n `mod` 2 == 0 = [Branch 0 tx ty
												| (x, y) <- [((n - 1) `div` 2, ((n - 1) `div` 2) + 1), (((n - 1) `div` 2) + 1, (n - 1) `div` 2)]
												, tx <- balTree x, ty <- balTree y]
							| otherwise      = [Branch 0 tx ty
												| tx <- balTree ((n - 1) `div` 2)
												, ty <- balTree ((n - 1) `div` 2)]
														 
symBalTrees					:: Int -> [Tree Int]
symBalTrees					= filter symmetric . balTree