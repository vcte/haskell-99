...
-- (*) Multiway trees - count nodes
-- nodesTree tree2
-- 2

-- (**) Tree construction from a node string
-- Tree> strToTree "afg^^c^bd^e^^^"
...
strToTree (x:xs)			= Node  x  ys
							where 
								z = map fst $ filter ((==) 0 . snd) $ zip [0..] $ 
									scanl (+) 0 $ map conv xs
								ys = map (strToTree . uncurry (sub xs)) $ zip (init z) (tail z)
								conv '^' = -1
								conv  _  = 1
								sub s a b = take (b - a) $ drop a s

treeToStr					:: Tree Char -> String
treeToStr a					= treeToStr' [a]
							where
								treeToStr' []				= "^"
								treeToStr' [Node a xs] 	    = a : treeToStr' xs ++ "^"
								treeToStr' (Node a xs : ys) = a : treeToStr' xs ++ treeToStr' ys
								
								{-strToTree' [] = ([], "")
								strToTree' ('^':xs) = ([], xs)
								strToTree' [x] = ([Node x []], "")
								strToTree' (x:'^':xs) = (Node x [] : y, ys)
									where (y, ys) = strToTree' xs
								strToTree' (x:xs) = ([Node x y], ys)
									where (y, ys) = strToTree' xs-}